...
# # # # # # # # # # # # # # # # # # # # # # # # # # # -Routes-  # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
''' Add '''
@bp.route('/contrib/add', methods=['POST'])
@token_required
def contrib_add(current_user):
    ...
    try:
        if data_type.lower() == "printed":
            ...
            language = int(form.language.data) # int: Jawa
            source = int(form.source.data) # int: buku
            isbn = ""
            if form.isbn.data:
                isbn = form.isbn.data
            ...
            add_print = PrintedData(uid=get_uinfo.uid, stid=source, lid=language, title=title, isbn=isbn)
            ...
            if not ext_check:
                return make_response(jsonify(message="Unsupported file type", status=False), 109)
            ...
            check_lang = db.session.execute(db.select(Languages).filter_by(lid=language)).scalar()
...
''' Update '''
@bp.route('/contrib/update', methods=['POST'])
@token_required
def contrib_update(current_user):
    data_type = request.args.get('type')
    try:
        if data_type.lower() == "printed":
        ...
        print(e)
        return make_response(jsonify(status=False, message="Internal Server Error"), 500)    
    
    return make_response(jsonify(type=data_type, status=True, message="Updated Successfully"), 201)


''' Delete '''
@bp.route('/contrib/remove', methods=['DELETE'])
@token_required
def contrib_remove(current_user):
    try:
        ...
        if content_id is None or content_id == "":
            return make_response(jsonify(status=False, message="Please Provide Content ID"), 401)
        
        if content_type is None or content_type == "":
            return make_response(jsonify(status=False, message="Please Provide Content Type"), 401)
        ...
        if content_type.lower() == "printed":
            ...
            return make_response(jsonify(status=False, message="Sorry we could not understand your request :("), 401)
    ...
    return make_response(jsonify(status=True, message="Success Delete Contribution"), 200)


''' View '''
@bp.route('/contrib/view', methods=['POST'])
@token_required
def contrib_view(current_user):
    data_type = request.args.get('type')
    ...
    try:
        ...
        if data_type.lower() == "printed":
            mains = PrintedData.query.filter(PrintedData.uid==getuid.uid, PrintedData.status==1)
        elif data_type.lower() == "digital":
            mains = DigitalData.query.filter(DigitalData.uid==getuid.uid, DigitalData.status==1)
        else:
            return make_response(jsonify(message="Unknown data types", status=False), 415)
...
''' View Detail '''
@bp.route('/contrib/view/detail', methods=['POST'])
@token_required
def contrib_view_detail(current_user):
    data_type = request.args.get('type')
    getreq = request.get_json()
